...
- 摄像头安装姿态（由用户提供）:
  ...
  - Y 轴: 竖直向下。
  - X 轴: 水平向右。
- 注意事项: 按“常规”摆放编写的程序可能会遇到“左右摄像头设备索引与物理左右位置不一致”的情况（例如 `/dev/video0` 可能对应物理右侧摄像头），这属于设备映射/编号问题，不是图像左右翻转。用于多摄像头拼接或空间标定时，务必在初始化阶段通过标定图或已知参照物确认每个设备的物理位置与朝向；如有必要，在预处理阶段交换摄像头索引或调整坐标映射以保证一致性。
